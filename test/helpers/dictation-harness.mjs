...
export function createDictationHarness(source, { supported = true, storedLanguage = null, bookLanguage = null, delayedStart = false, delayedEnd = false } = {}) {
  const state = { requests: [], starts: 0, stops: 0, instances: 0, recognition: null, startError: null, stopError: null, ending: null, styles: [], documentEvents: {}, windowEvents: {} };
  ...
  const document = {
    ...
    addEventListener(type, fn) { (state.documentEvents[type] ||= new Set()).add(fn); },
    removeEventListener(type, fn) { state.documentEvents[type]?.delete(fn); }
  };
  const window = supported ? {
    SpeechRecognition: Recognition,
    addEventListener(type, fn) { (state.windowEvents[type] ||= new Set()).add(fn); },
    removeEventListener(type, fn) { state.windowEvents[type]?.delete(fn); }
  } : {
    addEventListener(type, fn) { (state.windowEvents[type] ||= new Set()).add(fn); },
    removeEventListener(type, fn) { state.windowEvents[type]?.delete(fn); }
  };
  ...
  return {
    ...
    error(error) { state.recognition.onerror?.({ error }); },
    visibility(value = 'hidden') {
      document.visibilityState = value;
      for (const fn of [...(state.documentEvents.visibilitychange || [])]) fn({ type: 'visibilitychange' });
    },
    pagehide() {
      for (const fn of [...(state.windowEvents.pagehide || [])]) fn({ type: 'pagehide' });
    }
  };
}
