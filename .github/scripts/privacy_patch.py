...
if 'splendoria-cookie-notice-v1' in worker:
    worker = worker.replace('splendoria-cookie-notice-v1', 'splendoria-cookie-notice-v2')

banner_fn = '''function cookieNotice() {
...
pat = re.compile(r'function cookieNotice\(\) \{.*?\n\}(?=\nfunction )', re.S)
worker, n = pat.subn(banner_fn, worker, count=1)
if n != 1:
    raise SystemExit(f'cookieNotice patch: expected 1 function, found {n}')

css_marker = '/* privacy-first-banner-v2 */'
if css_marker not in worker:
    css = r'''
...
'''
    ...
    if worker.count(anchor) != 1:
        raise SystemExit(f'privacy banner CSS anchor expected 1, found {worker.count(anchor)}')
    worker = worker.replace(anchor, '\n' + css + anchor, 1)

worker_path.write_text(worker, encoding='utf-8')

if 'const SPL_PRIVACY_CENTER_PAGES = {' not in studio:
    legal = r'''
...
<section><h2>1. Titolare del trattamento</h2><p>Il Titolare è <strong>Raoul Ragazzi</strong>, Partita IVA 02950290219, Via Settala 22–24, Milano (MI). Per privacy, esercizio dei diritti o segnalazioni: <a href="mailto:contatti@splendoria.vip">contatti@splendoria.vip</a>.</p></section>
...
<section><h2>7. Titolare e diritti</h2><p>Titolare: Raoul Ragazzi, Partita IVA 02950290219, Via Settala 22–24, Milano (MI). Contatto: <a href="mailto:contatti@splendoria.vip">contatti@splendoria.vip</a>. Per finalità, basi giuridiche, destinatari e diritti si rinvia alla <a href="/privacy-policy">Privacy Policy</a>.</p></section>
</div></div></section>`
};
...
'''
    anchor = 'async function patchedFetch(request, env, ctx) {'
    if studio.count(anchor) != 1:
        raise SystemExit(f'legal page injection anchor expected 1, found {studio.count(anchor)}')
    studio = studio.replace(anchor, legal + '\n' + anchor, 1)

branch_marker = "  const contentType = response.headers.get('content-type') || '';\n"
privacy_branch = branch_marker + r'''
...
'''
if "splPatchPrivacyCenterPage(html, url.pathname)" not in studio:
    if studio.count(branch_marker) != 1:
        raise SystemExit(f'privacy route branch anchor expected 1, found {studio.count(branch_marker)}')
    studio = studio.replace(branch_marker, privacy_branch, 1)

studio_path.write_text(studio, encoding='utf-8')

checks = {
    'privacy notice v2': 'splendoria-cookie-notice-v2' in worker,
    'privacy banner': 'Privacy, senza sorprese' in worker,
    'no profiling banner': 'Niente pubblicità, niente profilazione' in worker,
    'privacy banner css': 'privacy-first-banner-v2' in worker,
    'privacy policy date': 'Ultimo aggiornamento: 29 agosto 2026' in studio,
    'GDPR rights': 'accesso, rettifica, cancellazione, limitazione, portabilità, opposizione' in studio,
    'Garante': 'Garante per la protezione dei dati personali' in studio,
    '365 day local draft': '365 giorni dall’ultima modifica' in studio,
    'cookie technical only': 'strumenti tecnici necessari' in studio,
    'AI transparency link': '/trasparenza-ai' in studio,
}
failed = [name for name, ok in checks.items() if not ok]
print(checks)
if failed:
    raise SystemExit('Privacy guardrails failed: ' + ', '.join(failed))
