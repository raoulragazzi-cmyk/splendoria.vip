...
# 1) UI: one discreet Musa button under each of the three section editors.
...
# 2) Backend: optional section mode on the existing authenticated /genera route.
...
# Preserve the compiled line-separator syntax and add only the requested section as extra source context.
...
# sourceContext already includes submittedContent, so no prompt rewrite is required.
...
# studio.js is cached for one year, so a new query key is mandatory.
