...
# UI: one discreet Musa button under every generated section editor.
...
# Backend: section mode is an optional parameter of the existing authenticated chapter generator.
...
# The compiled Worker writes literal backslash-newline separators, so preserve the separator found in source.
count_re = re.compile(r'(  const sourceForCount = museSourceMaterial\(project, \[\], \[interview\?\.answers, submittedContent)(\]\.filter\(Boolean\)\.join\([\s\S]*?\)\);)')
worker, n = count_re.subn(r'\1, sectionMode ? submittedSections[requestedSection] : ""\2', worker, count=1)
if n != 1:
    raise SystemExit(f'sourceForCount regex count={n}')
...
worker = worker[:gen_pos] + '  const sectionIndexes = sectionMode ? [requestedSection] : [0, 1, 2];\n  const generatedSections = [];\n' + worker[gen_pos + len(generated_anchor):]
...
# Insert the current section as an explicit drafting source, preserving the compiled line separator.
task_re = re.compile(r'(LUNGHEZZA: circa 350 parole, idealmente tra 300 e 400\.)(\\\n)(COERENZA CON L\'INDICE:)')
def task_repl(match):
    sep = match.group(2)
    return match.group(1) + sep + 'BOZZA ATTUALE DELLA SEZIONE (se presente, sviluppala senza cancellarne i fatti):' + sep + '${currentDraft || "Nessuna"}' + sep + match.group(3)
worker, n = task_re.subn(task_repl, worker, count=1)
if n != 1:
    raise SystemExit(f'task separator regex count={n}')

# Merge only the requested generated section back into the three submitted sections.
content_re = re.compile(r'  const content = collapseAccidentalRepetitions\(generatedSections\.join\((?P<sep>[\s\S]*?)\), 6e4\);')
m = content_re.search(worker)
if not m:
    raise SystemExit('content compose regex not found')
sep_expr = m.group('sep')
content_add = '  const content = collapseAccidentalRepetitions(sectionMode ? submittedSections.map((part, index) => index === requestedSection ? generatedSections[0] : part).join(' + sep_expr + ') : generatedSections.join(' + sep_expr + '), 6e4);'
worker = worker[:m.start()] + content_add + worker[m.end():]
...
# Browser cache for studio.js is one year, so a new query key is mandatory.
