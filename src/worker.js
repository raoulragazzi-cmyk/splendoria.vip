...
function renderBookPreview(p,chapters,authorName,user,back="/studio"){
  ...
  return page(p.title, `<section class="book-preview-shell"><div class="book-preview-toolbar"><a href="${esc(back)}">← Torna indietro</a><div><button class="button" type="button" data-print-book>Apri stampa / Salva PDF</button><p class="small muted">Formato tascabile 110 × 180 mm. Nella finestra di stampa scegli “Salva come PDF”, scala 100% e disattiva intestazioni e piè di pagina del browser.</p></div></div>${adminReview}<article class="book-volume" aria-label="Anteprima del libro impaginato"><section class="book-title-page"><p class="book-imprint">Splendoria</p><h1>${esc(p.title)}</h1><p class="book-author">di ${esc(authorName)}</p><p class="book-edition">Edizione personale</p></section><section class="book-toc"><p class="book-overline">Sommario</p><h2>Indice</h2><ol>${index}</ol></section>${chapterPages}</article></section>`, user, 200, bookPrintStyles());
}
...
async function adminDashboard(user, env, url) {
  ...
  const clientTable = clients.results.map(c => { const hasLegacy = Number(c.legacyChapters || 0) > 0 && Number(c.books || 0) === 0, chapters = hasLegacy ? Number(c.legacyChapters || 0) : Number(c.chapters || 0), completed = hasLegacy ? Number(c.legacyChapters || 0) : Number(c.completedChapters || 0), pct = chapters ? Math.round(completed / chapters * 100) : 0, books = Number(c.books || 0) + (hasLegacy ? 1 : 0), state = c.latestStatus || (hasLegacy ? "bozza storica" : "registrato · nessun libro"), pdf = c.latestProjectId ? `/admin/progetto/${c.latestProjectId}/anteprima` : hasLegacy ? `/admin/cliente/${c.id}/anteprima-storica` : ""; return `<tr><td><b>${esc(c.nome || "Senza nome")}</b><br><a href="mailto:${esc(c.email)}">${esc(c.email)}</a></td><td><span class="badge">${esc(state)}</span></td><td>${chapters ? `<div class="meter"><span style="width:${pct}%"></span></div><span class="small">${pct}% · ${completed}/${chapters}</span>` : "—"}</td><td>${books}</td><td>${c.orders}</td><td>${pdf ? `<a class="button" href="${pdf}" target="_blank" rel="noopener">Vedi PDF</a>` : `<span class="small muted">Nessun contenuto</span>`}</td><td>${new Date(c.createdAt).toLocaleDateString("it-IT")}</td></tr>`; }).join("");
