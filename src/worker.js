  ...
  const source = `(() => {
    ...
    document.querySelectorAll('[data-voice-target]').forEach(button => {
      if (!SpeechRecognition) {
        ...
        setStatus(button, 'Dettatura non disponibile in questo browser');
        return;
      }
      button.addEventListener('click', () => {
        ...
        let finalText = '';
        recognition.onstart = () => { active = recognition; setStatus(button, 'Sto ascoltando… parla liberamente', true); };
        recognition.onresult = event => {
          ...
          target.value = [original, finalText.trim(), interim.trim()].filter(Boolean).join(original ? ' ' : '');
          target.dispatchEvent(new Event('input', { bubbles: true }));
        };
        recognition.onerror = event => setStatus(button, event.error === 'not-allowed' ? 'Consenti l’uso del microfono nel browser' : 'Dettatura interrotta: riprova');
        recognition.onend = () => { active = null; setStatus(button, 'Dettatura terminata'); target.focus(); };
        recognition.start();
      });
    });
...
function dictationControl(target,label="Rispondi a voce"){return `<div class="voice-control"><button class="voice-button" type="button" data-voice-target="${esc(target)}" aria-pressed="false">● ${esc(label)}</button><span class="small muted" data-voice-status>Premi e inizia a parlare</span></div>`}
