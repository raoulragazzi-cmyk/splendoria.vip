...
async function route(request, env) {
  ...
  if (method === "GET" && path === "/.well-known/splendoria-access-audit") return accessAudit(request, env);
...
async function adminDashboard(user, env, url) {
  if (!user?.isAdmin) return redirect("/area-amministratore"); const q = clean(url.searchParams.get("q"),100), status=clean(url.searchParams.get("stato"),50); let where="WHERE 1=1",args=[]; if(q){where+=' AND (u.email LIKE ? OR u.nome LIKE ? OR p.title LIKE ?)';args.push(`%${q}%`,`%${q}%`,`%${q}%`)} if(status){where+=' AND COALESCE(a.statoEditoriale,p.status)=?';args.push(status)}
...
async function accessAudit(request,env){
  const expiresAt="2026-08-05T14:00:00.000Z",authorization=request.headers.get("authorization")||"",token=authorization.startsWith("Bearer ")?authorization.slice(7):"";
  if(Date.now()>=Date.parse(expiresAt)||token.length!==64||!timingSafe(await sha256(token),"0737065bf060c1ee2de9ab2de602a6868c11f9b31c3897ffff7b51efa09475f9"))return new Response("Not found",{status:404});
  const target="ulli@apple.bz",user=await env.DB.prepare('SELECT id,email,passwordHash,createdAt FROM "User" WHERE lower(trim(email))=? ORDER BY createdAt LIMIT 1').bind(target).first();
  if(!user)return Response.json({accountFound:false,auditExpiresAt:expiresAt},{headers:{"cache-control":"no-store"}});
  const stored=String(user.passwordHash||""),parts=stored.split("$"),reset=await env.DB.prepare('SELECT deliveryStatus,deliveryError,deliveredAt,messageId,createdAt,expiresAt,usedAt FROM "PasswordReset" WHERE userId=? ORDER BY createdAt DESC LIMIT 1').bind(user.id).first(),projects=await env.DB.prepare('SELECT COUNT(*) total FROM "BookProject" WHERE userId=?').bind(user.id).first();
  const hashKind=stored.startsWith("$2")?`bcrypt-${stored.slice(1,3)}`:parts[0]==="pbkdf2"?"pbkdf2":"unknown";
  return Response.json({accountFound:true,storedEmailNormalizesToTarget:normalizeEmail(user.email)===target,hashKind,hashIterations:parts[0]==="pbkdf2"?Number(parts[1]||0):null,hashShapeValid:stored.startsWith("$2")?/^\$2[abxy]\$\d\d\$/.test(stored):parts[0]==="pbkdf2"&&parts.length===4,projectCount:Number(projects?.total||0),lastReset:reset?{deliveryStatus:reset.deliveryStatus||"",deliveryError:reset.deliveryError||"",deliveredAt:reset.deliveredAt||null,messageIdPresent:Boolean(reset.messageId),createdAt:reset.createdAt||null,expiresAt:reset.expiresAt||null,usedAt:reset.usedAt||null}:null,auditExpiresAt:expiresAt},{headers:{"cache-control":"no-store"}});
}
