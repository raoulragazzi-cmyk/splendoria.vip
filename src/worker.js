...
async function saveChapter(request, projectId, chapterId, user, env) {
  ...
  if(!chapterUnlocked(p,chapter))return bookEditor(projectId,user,env,"Questo capitolo è riservato al libro completo. Potrai aprirlo dopo che Splendoria avrà registrato lo stato Pagato o Gratuito.");
...
async function generateAdaptiveChapter(request, projectId, chapterId, user, env) {
  ...
  if (!chapter) return redirect(`/libro/${projectId}`);
  if (!chapterUnlocked(project,chapter)) return bookEditor(projectId,user,env,"Questo capitolo è riservato al libro completo. Potrai aprirlo dopo che Splendoria avrà registrato lo stato Pagato o Gratuito.");
  ...
  if (!projectUnlocked(project)) { const used = await todayUsage(user.id, env); if (used >= FREE_AI_LIMIT) return bookEditor(projectId, user, env, "Hai usato le generazioni gratuite. Scegli una formula per continuare."); }
