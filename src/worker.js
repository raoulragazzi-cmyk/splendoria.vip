...
async function refineChapter(request,projectId,chapterId,user,env){if(!user)return redirect("/accedi");const p=await ownProject(projectId,user,env);if(!p)return redirect("/studio");const c=await env.DB.prepare('SELECT * FROM "BookChapter" WHERE id=? AND projectId=?').bind(chapterId,projectId).first();if(!c)return redirect(`/libro/${projectId}`);const f=await form(request),source=clean(f.content,60000)||c.content,instructions={grammar:"Correggi esclusivamente ortografia, grammatica, punteggiatura, concordanze e refusi. Non abbellire, non riassumere e non cambiare lessico, ritmo o voce dell'autore.",clarity:"Migliora chiarezza e scorrevolezza, sciogliendo frasi ambigue e ripetizioni, senza cambiare tono, fatti o personalità dell'autore.",emotional:"Rendi il testo più emozionante ma mai melodrammatico; valorizza sentimenti già presenti.",vivid:"Rendi le scene più vive e sensoriali usando soltanto dettagli presenti o formulazioni non fattuali.",elegant:"Rendi lo stile più elegante, fluido e letterario senza alterare i fatti o la voce dell'autore.",short:"Riduci il testo del 25%, elimina ripetizioni e mantieni i passaggi essenziali."},instruction=instructions[f.action]||instructions.grammar;if(!source)return redirect(`/libro/${projectId}`);let content;try{const ai=await env.AI.run("@cf/meta/llama-3.1-8b-instruct-fast",{prompt:`Agisci come editor italiano professionista. ${instruction} Mantieni sempre nomi, fatti, significato e punto di vista. Non inventare informazioni. Restituisci soltanto il testo revisionato, senza introduzioni, commenti o virgolette.\n\n${source}`,max_tokens:3500});content=String(ai.response||source).trim()}catch{content=source}await env.DB.prepare('UPDATE "BookChapter" SET content=?,status=?,updatedAt=? WHERE id=?').bind(content,`revisionato_${clean(f.action,30)||"grammar"}`,new Date().toISOString(),chapterId).run();return redirect(`/libro/${projectId}`)}
