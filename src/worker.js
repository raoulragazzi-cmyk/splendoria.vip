...
function renderBookPreview(p,chapters,authorName,user,back="/studio"){
  ...
  return page(p.title, `<section class="book-preview-shell"><div class="book-preview-toolbar"><a href="${esc(back)}">← Torna indietro</a><div><button class="button" type="button" data-print-book>Apri stampa / Salva PDF</button><p class="small muted">Formato Royal: taglio finito 155,6 × 233,9 mm, ricavato dal PDF di riferimento. Il file di stampa include 3 mm di abbondanza e linee di taglio. Scegli “Salva come PDF”, scala 100% e disattiva intestazioni e piè di pagina del browser.</p></div></div>${adminReview}${cropMarks}<article class="book-volume" aria-label="Anteprima del libro impaginato"><section class="book-title-page"><p class="book-imprint">Splendoria</p><h1>${esc(p.title)}</h1><p class="book-author">di ${esc(authorName)}</p><p class="book-edition">Edizione personale</p></section><section class="book-toc"><p class="book-overline">Sommario</p><h2>Indice</h2><ol>${index}</ol></section>${chapterPages}</article></section>`, user, 200, bookPrintStyles());
}
